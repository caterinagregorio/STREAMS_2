...
prior = df["onset_prior"].fillna(0.5).clip(1e-6, 1-1e-6).astype("float32").values
...
def __getitem__(self, i):
  return self.x_prior[i], self.cond[i], self.y[i], self.a[i], self.b[i], self.lab[i]
...
if mode == "gauss":  return aug_prior_gauss(x, base=kw.get("prior_sigma_base", 0.02),
                                            extra=kw.get("prior_sigma_extra", 0.08))
if mode == "dropout":return aug_prior_dropout(x, p=kw.get("prior_dropout_p", 0.1))
if mode == "beta":   return aug_prior_beta(x, kappa=kw.get("prior_kappa", 30.0))
...
w_class = torch.where(target > 0.5,
...
def pairwise_separation_loss(mu, y, mask, margin=1.0, lambda_pull=0.01, lambda_push=0.05, subsample_max=256):
  device = mu.device
lab = (mask > 0.5).squeeze(1)
if lab.sum() < 2: return torch.tensor(0.0, device=device)
...
M = torch.ones_like(d, dtype=torch.bool); M.fill_diagonal_(False)
same = (yy.unsqueeze(1) == yy.unsqueeze(0)) & M
diff = (~same) & M
pull = d2[same].mean() if same.any() else torch.tensor(0.0, device=device)
push = F.relu(margin - d[diff]).mean() if diff.any() else torch.tensor(0.0, device=device)
...
for x, cond, y, a, b, lab in loader:
  x, cond, y, a, b, lab = x.to(device), cond.to(device), y.to(device), a.to(device), b.to(device), lab.to(device)
...
# -----------------------------
def train_all(
  model_path, input_path, train_path, val_path, covariate_cols,
  latent_dim, max_epochs,
  batch_size=256, lr=2e-3, weight_decay=1e-4,
  warmup_epochs=50, max_beta=0.5,
  lambda_sd=0.01, lambda_pair_pull=0.1, lambda_pair_push=0.1, pair_margin=1.0,
  # New/missing args
  ...
  # FixMatch
  tau_pos=0.95, tau_neg=0.95, r_all=0.25,
  ema_alpha=0.99, lambda_fix_max=1.0, fix_ramp_epochs=40,
  # Prior augmentation
  prior_aug="gauss", prior_sigma_base=0.01, prior_sigma_extra=0.06,
  prior_dropout_p=0.10, prior_kappa=30.0,
  # Uncertainty gate
  K_aug=4, v_max=1e-3,
  # Early stop
  early_stop_patience=12, early_stop_warmup=50, seed=42
):
...
sel_num = 0.0;  sel_pos_num = 0.0;sel_neg_num = 0.0;unl_num = 0.0
...
x_aug = apply_prior_aug(
  x, prior_aug,
  prior_sigma_base=prior_sigma_base, prior_sigma_extra=prior_sigma_extra,
  prior_dropout_p=prior_dropout_p, prior_kappa=prior_kappa
)
...
L_pair = pairwise_separation_loss(
  mu, y, lab, margin=pair_margin,
  lambda_pull=lambda_pair_pull, lambda_push=lambda_pair_push
)
...
pt_mean, pt_var = teacher_stats_over_aug(
  teacher, x, cond, a, b, K=K_aug, prior_aug=prior_aug, prior_kw=prior_kw
)
unl = (1.0 - lab)
...
mask_u_pos = (sel_pos * (1.0 - lab)).detach()  # only unlabeled confident positives
...
L_age_cons = (
  ((mu_age_s - mu_age_t).pow(2) + (torch.log(sd_age_s) - torch.log(sd_age_t)).pow(2)) * mask_u_pos
).sum() / den_c
...
sel_num += float(sel.sum()); unl_num += float((1.0 - lab).sum())
...
nB = max(len(dl_tr), 1)
for k in logs: logs[k] /= nB
sel_rate = sel_num / max(unl_num, 1e-6) if unl_num > 0 else 0.0
print(f"Epoch {epoch}/{max_epochs} " f"| sup_bce:{logs['sup_bce']:.4f} | age:{logs['age']:.4f} | age_u:{logs['age_u']:.4f} "
...
print(f"VAL - total:{val['total']:.4f} | bce:{val['bce']:.4f} | age:{val['age']:.4f} | sd:{val['sdreg']:.4f}")
...
# FixMatch onset
ap.add_argument("--r_all", type=float, default=0.3,
...
train_all(
  args.model_path, args.input_path, args.train_split_path, args.val_split_path,
  covariate_cols, args.latent_dim, args.max_epochs,
  batch_size=args.batch_size, lr=args.lr, weight_decay=args.weight_decay,
  warmup_epochs=args.warmup_epochs, max_beta=args.max_beta,
  lambda_sd=args.lambda_sd, lambda_pair_pull=args.lambda_pair_pull,
  lambda_pair_push=args.lambda_pair_push, pair_margin=args.pair_margin,
  lambda_age_sup=args.lambda_age_sup, lambda_age_u=args.lambda_age_u,
  age_u_ramp_epochs=args.age_u_ramp_epochs, lambda_age_cons=args.lambda_age_cons,
  tau_pos=args.tau_pos, tau_neg=args.tau_neg, r_all=args.r_all,
  ema_alpha=args.ema_alpha, lambda_fix_max=args.lambda_fix_max, fix_ramp_epochs=args.fix_ramp_epochs,
  prior_aug=args.prior_aug, prior_sigma_base=args.prior_sigma_base, prior_sigma_extra=args.prior_sigma_extra,
  prior_dropout_p=args.prior_dropout_p, prior_kappa=args.prior_kappa,
  K_aug=args.K_aug, v_max=args.v_max,
  early_stop_patience=args.early_stop_patience, early_stop_warmup=args.early_stop_warmup, seed=args.seed
)
